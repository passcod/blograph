...
pub fn from_path(path: &PathBuf) -> Option<DateTime<UTC>> {
    ...
    match DATE.find_iter(
        &String::from(path.to_str().unwrap()).to_lowercase()
    ).last() {
        None => None,
...
mod test {
    ...
    fn strip_front_slash() {
        assert_eq!(strip(
            &PathBuf::from("/2017-feb-18-hello")),
            String::from("hello")
        );
    }
    ...
    fn strip_two_dates() {
        assert_eq!(strip(
            &PathBuf::from("2017-feb-18-and-2016-jul-03-hello-world.md")),
    ...
    fn strip_two_dates_subpath() {
        assert_eq!(strip(
            &PathBuf::from("2017-feb-18-hello/2016-jul-03-world.md")),
